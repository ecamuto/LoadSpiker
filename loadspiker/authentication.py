...
class BearerTokenAuthenticationFlow(AuthenticationFlow):
    ...
    def _fetch_token_from_endpoint(self, engine, user_id: Union[str, int], 
        ...
        # Store token. Do NOT echo the raw token_response (it carries the
        # access + refresh tokens) — expose only non-secret metadata so the
        # result dict is safe to log.
        result = self._set_bearer_token(user_id, access_token, expires_in)
        result['token_type'] = token_response.get('token_type')
        result['scope'] = token_response.get('scope')
        result['has_refresh_token'] = 'refresh_token' in token_response
...
class OAuth2AuthorizationCodeFlow(AuthenticationFlow):
    ...
    def _exchange_code_for_token(self, engine, user_id: Union[str, int],
        ...
        # Validate the CSRF state parameter returned by the authorization
        # server against the value we generated. Only enforced when a state
        # was stored (i.e. the auth URL was generated by this flow).
        expected_state = session.get('oauth2_state')
        if expected_state:
            returned_state = kwargs.get('state')
            if returned_state is None:
                raise AuthenticationError("Missing OAuth2 state parameter on code exchange")
            if not secrets.compare_digest(str(returned_state), str(expected_state)):
                raise AuthenticationError("OAuth2 state mismatch — possible CSRF")
